...
void grcentp(TGraph*gr, int type){  
  int N = gr->GetN();
  double *X = gr->GetX();
  switch (type) {
  case 0:
...
char *atypes[][6]={
  {"#Sigma E_{T} [TeV]","N_{ch}^{rec}", "Centrality [%]",  "#Sigma E_{T} [TeV]","N_{ch}^{rec}", "Centrality [%]"},
  {"#LT#Sigma E_{T}#GT/(4.1TeV)","#LTN_{ch}^{rec}#GT/2800", "Centrality [%]","#LTN_{ch}^{rec}#GT/2800","#LT#Sigma E_{T}#GT/(4.1TeV)","Centrality [%]"},
  //  {"#Sigma E_{T}/(4.1TeV)","N_{ch}^{rec}/2800", "Centrality [%]","#LTN_{ch}^{rec}#GT/2800","#LT#Sigma E_{T}#GT/(4.1TeV)","Centrality [%]"},
};
...
double lims[] ={5.1, 3800,  1.02, 5.1, 3600,  1.02};
...
int sty[]={20,24,25,27,28,30,31};
int col[] ={1,2,6,4,3,434,8};
int fcol[]={16,623,607,591,407,422,8};
...
void draw2();
TGraph*grcms,	* grcms1;      
void drawcumu5sidebyside5(int type=1){//Cent or FCal
  ...
  sprintf(name,"100*(%f-x)",lims[2]);
  //funA=new TF1("funA","(x)",100*lims0[2],100*lims[2]);
  funA=new TF1("funA","-x",100*lims0[2],100*lims[2]);//this determins the actual range of the mapped values.

  //funA=new TF1("funA","-x",100*(1-lims[2]),100*(1-lims0[2]));//this determins the actual range of the mapped values.

  A1 = new TGaxis(lims0[2],2,lims[2],2,"funA",505,"+");
  // A1->SetWmin(100*(1-lims[2]));	A1->SetWmax(100*(1-lims0[2]));

  A1->SetTitle("Centrality [%]");
  A1->SetTitleOffset(1.2);
  A1->SetLineWidth(0);
  A1->SetTitleFont(43);    A1->SetTitleSize(16);
  A1->SetLabelFont(43);    A1->SetLabelSize(14);
  A1->SetLineColor(2);
  A1->SetTickSize(0.); 
  A1->SetGridLength(0); A1->SetLineColor(0);
  ...
  TGraph*gmap[10];//mapping between different x-axes.
  f2 =new TFile("../data/paper/hist_cvt.root");
  gmap[0]= (TGraph*)f2->Get("gCvt_NchRec_FCal");
  gmap[1]= (TGraph*)f2->Get("gCvt_FCal_NchRec");
...
void  draw2(){
  ...
  for(int itt=0;itt<5;itt++){//Et, Nch and Cent=2
	...
	mmax1[ihar]=mmax;	mmin1[ihar]=mmin;
      ...
      if(itt==2){//modify the xaxis for centrality
	TH1*ttmp = htmp[itt]; 	gPad->Update();
	ttmp->GetXaxis()->SetTitleSize(0);	ttmp->GetXaxis()->SetLabelSize(0);
	A1->SetX1(ttmp->GetXaxis()->GetXmin());	A1->SetX2(ttmp->GetXaxis()->GetXmax());
	A1->SetY1(ttmp->GetMinimum());	A1->SetY2(ttmp->GetMinimum());
	A1->SetWmin(100*(1-ttmp->GetXaxis()->GetXmax()));	A1->SetWmax(100*(1-ttmp->GetXaxis()->GetXmin()));
	A1->DrawClone();	gPad->RedrawAxis();
      }
    ...
    sprintf(name,"figs5/%s.pdf",c1->GetName());	c1->Print(name);  c1->Write();
  }
  //vn2 ratios
  for(int it1=0;it1<2;it1++){//Nch/Et, Nch/et
 ...
 //vn{4}
  ...
  for(int itt=0;itt<3;itt++){//Et, Nch and Cent.
	...
	int N=g1[it][0]->GetN();	TGraphAsymmErrors*ggg = (TGraphAsymmErrors*)g1[it][0];
	double th=0.05; if(ihar==2) th=0.05; if(ihar==3) th=0.05;
	if(itt==2) {if(ihar==2) th=0.08; if(ihar==3) th=0.15;}
	while((ggg->GetEYlow()[N-1]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	  N--;
	  g1[it][0]->RemovePoint(N);
	  g1[it][1]->RemovePoint(N);
	...
	th=0.05; if(ihar==2) th=0.05; if(ihar==3) th=0.08;
	if(itt==2) {if(ihar==2) th=0.05; if(ihar==3) th=0.05;}
	while((ggg->GetEYlow()[0]>th)||(ggg->GetEYlow()[0]<1e-12)){//clean up some points
	  g1[it][0]->RemovePoint(0);
	  g1[it][1]->RemovePoint(0);
	}
	th=0.08; if(ihar==2) th=0.1; if(ihar==3) th=0.15;
	N=g1[it][0]->GetN();
	for(int ibin=N-1;ibin>=0;ibin--){
	  if(ggg->GetEYlow()[ibin]>th||ggg->GetEYhigh()[ibin]>th){
	    g1[it][0]->RemovePoint(ibin);
	...
	if(ihar==3) hh->GetXaxis()->SetLimits(0.36,lims[it1]);
      }
      if(itt==2){//modify the xaxis for centrality
	TH1*ttmp = hh; 	gPad->Update();
	ttmp->GetXaxis()->SetTitleSize(0);	ttmp->GetXaxis()->SetLabelSize(0);
	A1->SetX1(ttmp->GetXaxis()->GetXmin());	A1->SetX2(ttmp->GetXaxis()->GetXmax());
	A1->SetY1(ttmp->GetMinimum());	A1->SetY2(ttmp->GetMinimum());
	A1->SetWmin(100*(1-ttmp->GetXaxis()->GetXmax()));	A1->SetWmax(100*(1-ttmp->GetXaxis()->GetXmin()));
  ...
  for(int itt=0;itt<5;itt++){//Et, Nch and Cent.
    ...
    for(int ihar=1;ihar<NHAR;ihar++){
      ...
      gPad->SetTopMargin(0.01);
      for(int it=0;it<NPTS;it++){
	if(ihar==1){ if(it>3) ib=1; else ib=1;}
	if(ihar==2){ if(it>2) ib=1; else ib=1;}
	if(ihar==3){ if(itt==2) ib=1; else { if(it>2) ib=2; else ib=1;}}
	if(ihar==1){
	...
	if(ihar==3) htmp[it1]->GetXaxis()->SetLimits(0.3,lims[it1]);
      ...
      if(it1==2){//modify the xaxis for centrality
	TH1*ttmp = htmp[it1]; 	gPad->Update();
	ttmp->GetXaxis()->SetTitleSize(0);	ttmp->GetXaxis()->SetLabelSize(0);
	A1->SetX1(ttmp->GetXaxis()->GetXmin());	A1->SetX2(ttmp->GetXaxis()->GetXmax());
	A1->SetY1(ttmp->GetMinimum());	A1->SetY2(ttmp->GetMinimum());
	A1->SetWmin(100*(1-ttmp->GetXaxis()->GetXmax()));	A1->SetWmax(100*(1-ttmp->GetXaxis()->GetXmin()));
  ...
  for(int itt=0;itt<2;itt++){//Et, Nch and Cent.
	...
	if(ihar==1) {ib1=0; if(it>2) ib1=1;} if(ihar==2) {ib1=2; if(it>2) ib1=3;} if(ihar==3) {ib1=2; if(it>2) ib1=3;}
	if(ihar==1) {ib2=0; if(it>2) ib2=1;} if(ihar==2) {ib2=2; if(it>2) ib2=3;} if(ihar==3) {ib2=2; if(it>2) ib2=3;}
	gPad->SetTopMargin(0.01);
	for(int iw=0;iw<2;iw++){
	  if(iw==0){//ET
	    g1b[it][iw][0] =  (TGraph*)hr4[ib1][ihar][iet][pts[it]][0]->Clone();
	    g1b[it][iw][1] =  (TGraph*)hr4[ib1][ihar][iet][pts[it]][1]->Clone();
	  }else{//Nch
	    g1b[it][iw][0] =  (TGraph*)hr4[ib2][ihar][ich][pts[it]][0]->Clone();
	    g1b[it][iw][1] =  (TGraph*)hr4[ib2][ihar][ich][pts[it]][1]->Clone();
	  }
	  g1b[it][iw][0]->SetMarkerStyle(bsty[iw]); g1b[it][iw][0]->SetMarkerColor(bcol[iw]); g1b[it][iw][0]->SetLineColor(bcol[iw]);
	  g1b[it][iw][1]->SetMarkerSize(0); g1b[it][iw][1]->SetMarkerColor(bfcol[iw]); g1b[it][iw][1]->SetLineColor(bfcol[iw]);
	  g1b[it][iw][1]->SetLineWidth(5);
	  int N=g1b[it][iw][0]->GetN();	TGraphAsymmErrors*ggg = (TGraphAsymmErrors*)g1b[it][iw][0];
	  double th=0.05; if(ihar>1) th=0.06; if(iw) {th=0.07; if(ihar==2) th=0.08; if(ihar==3) th=0.16;}
	  while((ggg->GetEYlow()[N-1]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	    N--;
	    g1b[it][iw][0]->RemovePoint(N);
	    g1b[it][iw][1]->RemovePoint(N);
	  }
	  th=0.05; if(ihar==2) th=0.08; if(ihar==3) th=0.08;
	  if(iw) {th=0.06; if(ihar==2) th=0.08; if(ihar==3) th=0.15;}
	  while((ggg->GetEYlow()[0]>th)||(ggg->GetEYlow()[0]<1e-12)){//clean up some points
	    g1b[it][iw][0]->RemovePoint(0);
	    g1b[it][iw][1]->RemovePoint(0);
	  }
	  if(itt==0) { ran1=0.6/Knee[0];ran2=4.4/Knee[0]; } else {ran1=400/Knee[1];ran2=3000/Knee[1];}
	  GetMax(g1b[it][iw][0]); if(m1>mmax) mmax = m1; if(m2<mmin) mmin = m2;
	}
	if(itt==0){//backup
	  mmax1[ihar]=mmax;	mmin1[ihar]=mmin;
	}
	...
	l->SetFillStyle(0);  l->SetBorderSize(0);
	l->SetTextFont(43);  l->SetTextSize(15);
	sprintf(name,"nc_{%d}{4,#Sigma E_{T}}",ihar+1); l->AddEntry(g1b[it][0][0],name,"pl");  
	sprintf(name,"nc_{%d}{4,N_{ch}^{rec}}",ihar+1); l->AddEntry(g1b[it][1][0],name,"pl");  
	l->Draw();
	...
	c1 = ct[it]; sprintf(name,"figs5/%s.pdf",c1->GetName());	c1->Print(name);  c1->Write();
      }
      //show more pT bins together
      sprintf(name,"comp_nc4comp_t%d_har%d_all",itt,ihar+1);   c1 = new TCanvas(name,name,400,400);Divide(c1,1,1,0.,0.15);
      gPad->SetTopMargin(0.01);
      for(int it=0;it<2;it++){
	for(int iw=0;iw<2;iw++){
	  g2b[it][iw][0]=  (TGraph*)g1b[ptss[it]][iw][0]->Clone();
	  g2b[it][iw][1]=  (TGraph*)g1b[ptss[it]][iw][1]->Clone();
	  g2b[it][iw][0]->SetMarkerStyle(csty[it][iw]); g2b[it][iw][0]->SetMarkerColor(ccol[it][iw]); g2b[it][iw][0]->SetLineColor(ccol[it][iw]);
	  g2b[it][iw][1]->SetMarkerSize(0); g2b[it][iw][1]->SetMarkerColor(cfcol[it][iw]); g2b[it][iw][1]->SetLineColor(cfcol[it][iw]);
	  g2b[it][iw][1]->SetLineWidth(5);
	}
      }
      TH1*hh = (TH1*)hcoora[itt]->DrawClone();
      hh->SetMaximum(mmax1[ihar]*2); hh->SetMinimum(mmin1[ihar]*1.32);
      sprintf(name,"nc_{%d}{4}",ihar+1);
      hh->GetYaxis()->SetTitle(name);      hh->GetXaxis()->SetTitleOffset(1.);
      g2b[0][0][1]->Draw("P");
      g2b[0][1][1]->Draw("P");
      ...
      double shiy=0,shix=0.4;
      if(ihar==1){shix=0.;shiy=-0.2;}
      if(ihar==2){shix=0.3;shiy=-0.35;}
      if(ihar==3){shix=0.3;shiy=-0.3;}
      sprintf(name,"nc_{%d}{4,#SigmaE_{T}}",ihar+1);   lab2.DrawLatex(0.17+shix,0.90+shiy,name);
      sprintf(name,"nc_{%d}{4,N_{ch}^{rec}}",ihar+1);  lab2.DrawLatex(0.33+shix,0.90+shiy,name);
      // lab1.DrawLatex(0.18+shix,0.90+shiy,"#Sigma E_{T}");lab1.DrawLatex(0.28+shix,0.90+shiy,"N_{ch}^{rec}");
      l = new TLegend(0.18+shix, 0.67+shiy, 0.68+shix, 0.88+shiy);
      l->SetFillStyle(0);  l->SetBorderSize(0);
      l->SetTextFont(43);  l->SetTextSize(14);
      l->AddEntry(g2b[0][0][0]," ","pl");  
      l->AddEntry(g2b[1][0][0]," ","pl");  
      l->Draw();

      l = new TLegend(0.33+shix, 0.67+shiy, 0.83+shix, 0.88+shiy);
      l->SetFillStyle(0);  l->SetBorderSize(0);
      l->SetTextFont(43);  l->SetTextSize(13);
      l->AddEntry(g2b[0][1][0],ptname1[ptsslab[0]],"pl");  
      l->AddEntry(g2b[1][1][0],ptname1[ptsslab[1]],"pl");  
      l->Draw();
      ...
      lab1.DrawLatex(0.25+shix,0.24+shiy,"#it{#bf{ATLAS}} Internal"); //  lab2.DrawLatex(0.32, 0.42,"Preliminary");
      lab1.DrawLatex(0.25+shix,0.18+shiy,"Pb+Pb 5.02 TeV, 22-470 #mub^{-1}");
      if(ihar==3){text1.DrawLatex(0.35,0.92,"Standard method");	}      else {text1.DrawLatex(0.2,0.92,"Standard method");}
      sprintf(name,"figs5/%s.pdf",c1->GetName());	c1->Print(name);  c1->Write();
    }
  }
  ...
  for(int itt=0;itt<2;itt++){//Et, Nch and Cent.
	...
	ib1=0;ib2=0; if(it>1){ ib1=1;ib2=1;}
	//ib1=1;ib2=1;
	gPad->SetTopMargin(0.01);
	for(int iw=0;iw<2;iw++){
	  if(iw==0){//ET
  ...
  for(int itt=0;itt<2;itt++){//Et, Nch and Cent.
	...
	for(int iw=0;iw<2;iw++){
	  if(iw==0){//ET
	    if(ihar==1){
	      g1b[it][iw][0] =  (TGraph*)hnac[ib1][2][iet][pts[it]][0]->Clone();
	      g1b[it][iw][1] =  (TGraph*)hnac[ib1][2][iet][pts[it]][1]->Clone();
	    }else{
	      g1b[it][iw][0] =  (TGraph*)hnsc[ib1][ihar][iet][pts[it]][0]->Clone();
	      g1b[it][iw][1] =  (TGraph*)hnsc[ib1][ihar][iet][pts[it]][1]->Clone();
	    }
	  }else{
	    if(ihar==1){
	      g1b[it][iw][0] =  (TGraph*)hnac[ib2][2][ich][pts[it]][0]->Clone();
	      g1b[it][iw][1] =  (TGraph*)hnac[ib2][2][ich][pts[it]][1]->Clone();
	    }else{
	      g1b[it][iw][0] =  (TGraph*)hnsc[ib2][ihar][ich][pts[it]][0]->Clone();
	      g1b[it][iw][1] =  (TGraph*)hnsc[ib2][ihar][ich][pts[it]][1]->Clone();
	    }
	  }
	  g1b[it][iw][0]->SetMarkerStyle(bsty[iw]); g1b[it][iw][0]->SetMarkerColor(bcol[iw]); g1b[it][iw][0]->SetLineColor(bcol[iw]);
	  g1b[it][iw][1]->SetMarkerSize(0); g1b[it][iw][1]->SetMarkerColor(bfcol[iw]); g1b[it][iw][1]->SetLineColor(bfcol[iw]);
	...
	l->AddEntry(g1b[it][1][0],name,"pl");        l->Draw();
...
void  draw1(){
  ...
  for(int ihar=1;ihar<NHAR;ihar++){
	...
	g1[it][0] =  (TGraph*)hv4[ib][ihar][it1][pts[it]][0]->Clone();
	g1[it][1] =  (TGraph*)hv4[ib][ihar][it1][pts[it]][1]->Clone();
	g2[it][0] =  (TGraph*)hr4[ib][ihar][it1][pts[it]][0]->Clone();//monitor
	int N=g1[it][0]->GetN();	TGraphAsymmErrors*ggg = (TGraphAsymmErrors*)g2[it][0];
	double th=0.05; if(ihar>1) th=0.03;
	if(it1<2){
	  while((ggg->GetEYlow()[N-1]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	    N--;
	...
	if(ihar==1){//
	  ...
	  N=g1[it][0]->GetN()-1;
	  if(it==0){
	    g1[it][0]->RemovePoint(N);   g1[it][1]->RemovePoint(N);
	    g1[it][0]->RemovePoint(N-1); g1[it][1]->RemovePoint(N-1);
	  }
	  if(it==1){
	    g1[it][0]->RemovePoint(N); g1[it][1]->RemovePoint(N);
	  }
	}
	if(it1==2&&ihar==3&&it==0){
	  g1[it][0]->Print();
	}
	ggg = (TGraphAsymmErrors*)g1[it][0]; N=g1[it][0]->GetN();
	if(ihar>1){
	  if(ihar==2) th=0.02; if(ihar==3) th=0.03;
	  if(it1<2){
	    while((ggg->GetEYlow()[N-1]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	      N--;
	      g1[it][0]->RemovePoint(N);
	    ...
	    while((ggg->GetEYhigh()[N-1]>th)||(ggg->GetEYhigh()[N-1]<1e-12)){//clean up some points
	      N--;
	      g1[it][0]->RemovePoint(N);
	      g1[it][1]->RemovePoint(N);
	    }
	    ...
	    while((ggg->GetEYlow()[0]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	      g1[it][0]->RemovePoint(0);
	      g1[it][1]->RemovePoint(0);
	    }
	    while((ggg->GetEYhigh()[0]>th)||(ggg->GetEYhigh()[N-1]<1e-12)){//clean up some points
	      g1[it][0]->RemovePoint(0);
	      g1[it][1]->RemovePoint(0);
	    }
	    if(ihar==2) th=0.02; if(ihar==3) th=0.02;
	    while((ggg->GetEYlow()[N-1]>th)||(ggg->GetEYlow()[N-1]<1e-12)){//clean up some points
	      N--;
	      g1[it][0]->RemovePoint(N);
	      g1[it][1]->RemovePoint(N);
	...
	if(ihar==3&&it==2&&it1==1){
	  N=g1[it][0]->GetN()-1;g1[it][0]->RemovePoint(N);   g1[it][1]->RemovePoint(N);
	}
	if(it1==0) { ran1=3.6/Knee[0];ran2=4.4/Knee[0]; } else if(it1==1) {ran1=2400/Knee[1];ran2=3000/Knee[1];}else {ran1=0.5;ran2=0.99;}
	GetMax(g1[it][0]); if(m1>mmax) mmax = m1; if(m2<mmin) mmin = m2;
	...
	if(ihar==3)htmp[it1]->GetXaxis()->SetLimits(0.32,lims[it1]);
      }
      htmp[it1]->SetMaximum(mmax);  htmp[it1]->SetMinimum(mmin);	
	...
	l->Draw();
      ...
      if(ihar>2)shix=0.28;
      text1.DrawLatex(0.2,0.9,ctypes[0]);
      lab1.DrawLatex(0.18+shix,0.09+shif,"#it{#bf{ATLAS}} Internal"); //  lab2.DrawLatex(0.32, 0.42,"Preliminary");
      lab1.DrawLatex(0.18+shix,0.02+shif,"Pb+Pb 5.02 TeV, 22-470 #mub^{-1}");
    }
    for(int it1=0;it1<NT1;it1++){//Et, Nch and Cent.
      if(ihar==1) htmp[it1]->SetMaximum(mmax*1.2); else if(ihar==2) htmp[it1]->SetMaximum(mmax*1.3); else  htmp[it1]->SetMaximum(mmax*2.5); 
      if(ihar<3) htmp[it1]->SetMinimum(mmin*1.3); else  htmp[it1]->SetMinimum(mmin*1.45); 
      if(it1==2){//modify the xaxis for centrality
	int pa = pads[it1]; 	c1->cd(pa+1);	
	TH1*ttmp = htmp[it1]; 	gPad->Update();
	ttmp->GetXaxis()->SetTitleSize(0);	ttmp->GetXaxis()->SetLabelSize(0);
  ...
  for(int ihar=1;ihar<NHAR;ihar++){
	...
	for(int it=0;it<NPTS;it++){
	  if(ihar==1){ if(it1==2) ib=0; else ib=0; if(it1==2) ibb=0; else ibb=0; if(it>2) {ib+=1; ibb+=1;}}
	  if(ihar==2){ if(it1==2) ib=2; else ib=2; if(it1==2) ibb=2; else ibb=2; if(it1==0) {if(it>2) {ib+=1; ibb+=1;}}}
	  if(ihar==3){ if(it1==2) ib=2; else ib=1; if(it1==0&&it>2) ibb=3; else ibb=2;}
	  if(iw==0){
	    g1[it][0] =  (TGraph*)hr4[ib][ihar][it1][pts[it]][0]->Clone();
	    g1[it][1] =  (TGraph*)hr4[ib][ihar][it1][pts[it]][1]->Clone();
	  }else{
  ...
  for(int ihar=2;ihar<NHAR;ihar++){
	...
	GetMax(g1[it][0]); if(m1>mmax) mmax = m1; if(m2<mmin) mmin = m2;
      ...
      if(it1==1)  sprintf(name,"%s{4,N_{ch}^{rec}}",scnames[ihar]); else  sprintf(name,"%s{4,#Sigma E_{T}}",scnames[ihar]);
       htmp[it1]->GetYaxis()->SetTitle(name);      htmp[it1]->GetXaxis()->SetTitleOffset(1.);
      if(it1==2){
  ...
  for(int ihar=2;ihar<3;ihar++){
	...
	htmp[iw] = (TH1*)hcoora[it1]->DrawClone();
	htmp[iw]->SetMaximum(mmax);  htmp[it1]->SetMinimum(mmin);	
	//if(it1==1)  sprintf(name,"%s{3,N_{ch}^{rec}}",acnames[2]); else  sprintf(name,"%s{3,#Sigma E_{T}}",acnames[2]);
	if(it1==1)  sprintf(name,"n%s{3,N_{ch}^{rec}}",acnames[ihar]); else  sprintf(name,"n%s{3,#Sigma E_{T}}",acnames[ihar]);
	htmp[iw]->GetYaxis()->SetTitle(name);      htmp[iw]->GetXaxis()->SetTitleOffset(1.);
	if(it1==2){
	  if(ihar==2) htmp[iw]->GetXaxis()->SetLimits(0.2,lims[it1]);
	  if(ihar==3) htmp[iw]->GetXaxis()->SetLimits(0.2,lims[it1]);
	...
	lab1.DrawLatex(0.25+shix,0.18+shiy,"Pb+Pb 5.02 TeV, 22-470 #mub^{-1}");
	if(ihar==2) shix=0.1;
	text1.DrawLatex(0.2+shix,0.87,ctypes[iw]);
      }
      if(mmin>0) mmin=0;
      for(int iw=0;iw<2;iw++){//Et, Nch and Cent.
	if(ihar==3) htmp[iw]->SetMaximum(mmax*1.5); else if(ihar==2) htmp[iw]->SetMaximum(mmax*1.5); else  htmp[iw]->SetMaximum(mmax*1.3);  
