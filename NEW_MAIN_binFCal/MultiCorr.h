...
class MultiCorr
{
	private:
		...
		double cal_4psc_3sub_Type1(Event* evt, int iV, int iP); // 4-particle symmetric correlation
		double cal_4psc_3sub_Type2(Event* evt, int iV, int iP); // 4-particle symmetric correlation
		...
		void swap(double&, double&); // swap double for 3-subevent
		void swap(TComplex&, TComplex&); // swap complex for 3-subevent
		void swap(Event*& evt1, Event*& evt2); // swap event for background
	...
	public:
		...
		TH1D* psc4_3sub_Type1_mean[NA][NV][NP];
		TH1D* psc4_3sub_Type2_mean[NA][NV][NP];
