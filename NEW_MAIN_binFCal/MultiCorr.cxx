	...
	for(unsigned int iP=0; iP<NP; iP++)
	{
		for(unsigned int iA=0; iA<NA; iA++)
		{
			...
			for(unsigned int iV=0; iV<NV; iV++)
			{
				...
				initHist_AVP(psc4_3sub_Type1_mean[iA][iV][iP],"psc4_3sub_Type1_mean",iA,iV,iP);
				initHist_AVP(psc4_3sub_Type2_mean[iA][iV][iP],"psc4_3sub_Type2_mean",iA,iV,iP);
	...
	for(unsigned int iP=0; iP<NP; iP++)
	{
		for(unsigned int iA=0; iA<NA; iA++)
		{
			...
			for(unsigned int iV=0; iV<NV; iV++)
			{
				...
				delete psc4_3sub_Type1_mean[iA][iV][iP];
				delete psc4_3sub_Type2_mean[iA][iV][iP];
	...
	for(unsigned int iA=0; iA<NA; iA++)
	{
		for(unsigned int iP=0; iP<NP; iP++)
		{
			...
			if(w2pc1>1e-6 && w2pc2>1e-6 && w3pc>1e-6 && w4pc>1e-6)
			{
				...
				for(unsigned int iV=0; iV<NV; iV++)
				{
					...
					psc4_3sub_Type1_mean[iA][iV][iP]->Fill(evt->evtCls(),evt->evtWght()*w4pc*cal_4psc_3sub_Type1(evt,iV,iP));
					psc4_3sub_Type2_mean[iA][iV][iP]->Fill(evt->evtCls(),evt->evtWght()*w4pc*cal_4psc_3sub_Type2(evt,iV,iP));
		...
		if(iA%3==2)
		{
			for(unsigned int iP=0; iP<NP; iP++)
			{
				for(unsigned int iW=0; iW<NW; iW++)
				{
					swap(evt->nTrk3sub[iP][0][iW],evt->nTrk3sub[iP][1][iW]);
					for(unsigned int iV=0; iV<NVm; iV++)
					{
						swap(evt->Q3sub[iV][iP][0][iW],evt->Q3sub[iV][iP][1][iW]);
					}
				}
			}
		}
	...
	for(unsigned int iP=0; iP<NP; iP++)
	{
		for(unsigned int iA=0; iA<NA; iA++)
		{
			...
			for(unsigned int iV=0; iV<NV; iV++)
			{
				...
				psc4_3sub_Type1_mean[iA][iV][iP]->Write();
				psc4_3sub_Type2_mean[iA][iV][iP]->Write();
...
double MultiCorr::cal_4psc_3sub_Type1(Event* evt, int iV, int iP)
{
	TComplex M11;
	TComplex N11;
	TComplex MN2;
...
double MultiCorr::cal_4psc_3sub_Type2(Event* evt, int iV, int iP)
{
	TComplex M11;
	TComplex N11;
	TComplex MN2;
	...
	double b11 = pow(evt->nTrk3sub[iP][1][1],1);
	double c11 = pow(evt->nTrk3sub[iP][2][1],1);
	return ((B11*TComplex::Conjugate(M11)*N11*TComplex::Conjugate(C11)).Re()-(MN2*B11*TComplex::Conjugate(C11)).Re())/((a21-a12)*b11*c11);
}
...
void MultiCorr::swap(double& val1, double& val2)
{
	double tmp = 0;
	tmp = val1;
	val1 = val2;
...
void MultiCorr::swap(TComplex& val1, TComplex& val2)
{
	TComplex tmp;
	tmp = val1;
	val1 = val2;
...
void MultiCorr::swap(Event*& evt1, Event*& evt2)
{
	Event* tmp = new Event(*evt1);
	*evt1 = *evt2;
	*evt2 = *tmp;
